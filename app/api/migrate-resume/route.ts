  ...
  try {
    ...
    if (!upstashUrl || !upstashToken) {
      return NextResponse.json({
        ...
        details: 'Please add UPSTASH_VECTOR_REST_URL and UPSTASH_VECTOR_REST_TOKEN to your .env.local file'
      }, { status: 400 });
    }

    // Get resume data from request body or load from file
    ...
    if (body?.resumePath) {
      // Load from file path provided
      console.log(`📄 Loading resume from: ${body.resumePath}`);
      const resumeData = readFileSync(body.resumePath, 'utf-8');
      resume = JSON.parse(resumeData);
    } else if (body?.resume) {
      // Use resume data from request body
      console.log('📄 Using resume data from request body');
      resume = body.resume;
    } else {
      // Default: try to load from Downloads folder
      const defaultPath = 'C:\\Users\\Meera\\Downloads\\meera_mishra_resume_structured.json';
      console.log(`📄 Loading resume from default path: ${defaultPath}`);
      try {
        const resumeData = readFileSync(defaultPath, 'utf-8');
        resume = JSON.parse(resumeData);
      } catch (error: any) {
        return NextResponse.json({
          success: false,
          error: 'Could not load resume from default path',
          details: `File not found at ${defaultPath}. Please provide resumePath in request body.`,
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully migrated ${documents.length} documents from resume to vector database`,
    ...
    return NextResponse.json(
      { 
        success: false,
        error: 'Failed to migrate resume to vector database',
